...
db.exec(`
  ...
  -- ออเดอร์ยืม/เบิก (จับหลายรายการเป็น 1 ใบเหมือนตะกร้า)
...
// บัญชี guest — ใช้อ้างอิงในประวัติเมื่อคนที่ยังไม่ login จัดการคลัง
// active=0 = login ด้วยบัญชีนี้ไม่ได้ (ไม่มีรหัสผ่านที่ใช้ได้)
{
  const g = db.prepare("SELECT id FROM users WHERE username='guest'").get();
  if (!g) {
    db.prepare(
      "INSERT INTO users (username, password, fullname, role, active) VALUES ('guest','-','ผู้เยี่ยมชม','staff',0)"
    ).run();
  }
}
