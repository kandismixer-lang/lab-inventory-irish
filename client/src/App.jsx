...
function Shell({ me, onMe, guestName, onGuestName }) {
  ...
  const confirmId = async () => {
    ...
    try {
      ...
      // แจ้งผลชัดเจน กันงงว่ากดแล้วไม่มีอะไรเกิด — แยก "เจอ" กับ "ไม่พบรายการของบัตรนี้"
      toast(r?.name ? `ดึงของที่ยืมของ ${r.name} แล้ว` : 'ยืนยันแล้ว — ยังไม่พบรายการยืมของรหัสบัตรนี้ (เช็คว่าพิมพ์ตรงกับตอนยืม)');
    } catch (e) {
          ...
          <div className="userbox">
            {isGuest ? (
              <>
                ...
                <div className="hint" style={{ margin: '2px 0 6px' }}></div>
                <button
